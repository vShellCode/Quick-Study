...
void FramelessWindow::setWindowGeometry(const QPointF &pos)
{
    ...
    case TOPLEFT: set_geometry_func(this->old_size + QSize(offset.x(), offset.y()), this->old_pos - offset);

        break;
    case TOP: set_geometry_func(this->old_size + QSize(0, offset.y()), this->old_pos - QPointF(0,offset.y()));

        break;
    case TOPRIGHT: set_geometry_func(this->old_size - QSize(offset.x(), -offset.y()), this->old_pos - QPointF(0,offset.y()));

        break;
    case LEFT: set_geometry_func(this->old_size + QSize(offset.x(), 0), this->old_pos - QPointF(offset.x(), 0));

        break;
    case RIGHT: set_geometry_func(this->old_size - QSize(offset.x(), 0), this->position());
